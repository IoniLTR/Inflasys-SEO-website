...
$part8_projects = [];
$target_dir = __DIR__ . '/../nos-realisations'; // Chemin relatif
...
require_once __DIR__ . '/../components/categories/part-8.php';
?>
   -------------------------------------------------------------- */
?>
...
.cat-realisation-item {
    display: flex;
    flex-direction: column;
    width: calc(47% - 1vw); /* Un peu moins de 50% pour laisser place au bouton */
    position: relative;
    overflow: hidden;
    border-radius: 4px;
    background: black;
...
.cat-realisation-all:hover {
    background-color: white;
}

.cat-realisation-all div {
    font-size: 1.2vw;
    margin-top: 1vw;
}
.cat-realisation-all svg {
    width: 2vw;
...
@media (max-width: 1024px) {
    ...
    .cat-realisations-pattern { display: none; }
    ...
    .cat-realisation-all { 
        width: 100%; writing-mode: horizontal-tb; 
        flex-direction: row; padding: 15px; justify-content: center; gap: 10px;
    }
...
<section class="cat-realisations">
  ...
  <?php if (!empty($part8_pattern)): ?>
    <img class="cat-realisations-pattern" src="<?= htmlspecialchars($part8_pattern, ENT_QUOTES, 'UTF-8') ?>" alt="pattern">
  <?php endif; ?>

  <div class="cat-realisations-content">
    <?php foreach ($part8_projects as $project): ?>
        <a class="cat-realisation-item" href="<?= htmlspecialchars($project['href'], ENT_QUOTES, 'UTF-8') ?>">
          <img src="<?= htmlspecialchars($project['img'], ENT_QUOTES, 'UTF-8') ?>" alt="">
    ...
    <a class="cat-realisation-all" href="<?= htmlspecialchars($part8_all_link, ENT_QUOTES, 'UTF-8') ?>">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 7h10v10"></path><path d="M7 17 17 7"></path></svg>
      <div>Tout voir</div>
    </a>
  </div>
