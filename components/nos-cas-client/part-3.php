...
<?php
   ...
   require_once '../../components/nos-cas-client/part-3.php';
   ?>
-------------------------------------------------------------- */
?>
...
<style>
    ...
    .all-cas-client-2-content {
        display: grid;
        grid-template-columns: repeat(2, 1fr);
        gap: 20px;
    }

    .all-cas-client-2-card {
        position: relative;
        display: block;
        height: 350px;
        border-radius: 8px;
    ...
    .all-cas-client-2-card:hover {
        transform: translateY(-5px);
    }

    .all-cas-client-2-card img {
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
    ...
    .all-cas-client-2-card:hover img {
        transform: scale(1.05);
    }

    .all-cas-client-2-card::after {
        content: '';
        position: absolute;
        top: 0;
        left: 0;
    ...
    .all-cas-client-2-card:hover .icon-card-svg {
        fill: #3DA8FF;
    }
    ...
    @media (max-width: 1024px) {
        ...
        .all-cas-client-2-content { grid-template-columns: 1fr; }
        .all-cas-client-2-card { height: 300px; }
    }
</style>

<section class="all-cas-client-2">
    <h2><?= htmlspecialchars($all_cas_client_part_3_title ?? 'Autres Projets', ENT_QUOTES, 'UTF-8') ?></h2> 
    
    <img class="all-cas-client-2-pattern" src="uploads/pattern-1.webp" alt="" aria-hidden="true">
    
    <div class="all-cas-client-2-content">
        <?php if(isset($all_cas_client_part_3) && is_array($all_cas_client_part_3)): ?>
            <?php foreach ($all_cas_client_part_3 as $item): ?>
                <a href="<?php echo $base_url?><?php echo $item['link']; ?>" class="all-cas-client-2-card">
                    <img src="<?php echo $images_url?><?php echo $item['img']; ?>" alt="<?php echo htmlspecialchars($item['title']); ?>" loading="lazy">
                    <div class="card-desc-top-2"><?php echo $item['desc']; ?></div>
                    <div class="all-cas-client-2-card-text">
